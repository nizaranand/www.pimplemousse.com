...
?>
    <p>
        <label for="<?php echo $widget->get_field_id('title_font'); ?>"><?php _e('Title font', 'clean') ?></label>
        <select name="<?php echo $widget->get_field_name('title_font'); ?>" id="<?php echo $widget->get_field_id('title_font'); ?>" class="widefat">
            <option value="default" <?php echo selected($instance['title_font'], 'none') ?>><?php _e('Default font', 'clean') ?></option>
        <?php if ($customFonts != '') {
        ?>
            <optgroup label="Custom cufon fonts">
            <?php
            foreach ($customFonts['fonts'] as $fnt) {
            ?>
                <option value="<?php print $fnt['name']; ?>" <?php echo selected($instance['title_font'], $fnt['name']) ?>><?php print $fnt['name']; ?></option>
            <?php }
            ?>
        </optgroup>
        <?php } ?>
...
<p>
    <label for="<?php echo $widget->get_field_id('themeton_widget_showhide'); ?>"><?php _e('Show/hide widget', 'clean') ?></label>
    <select name="<?php echo $widget->get_field_name('themeton_widget_showhide'); ?>" id="<?php echo $widget->get_field_id('themeton_widget_showhide'); ?>">
        <option value="0" <?php echo selected($instance['themeton_widget_showhide'], '0'); ?>>Hide on selected</option>
        <option value="1" <?php echo selected($instance['themeton_widget_showhide'], '1'); ?>>Show on selected</option>
    </select>
...
<?php
        ...
        function clean_custom_widget_show($instance, $widget, $args) {
            ...
            $titleClass = '';
            $cufonRel = '';
            if ($instance['title_font'] != 'default'&&$instance['title_font']!='') {
                $titleClass = 'customCufon';
                $cufonRel = $instance['title_font'];
            }
            $args['before_title'] = '<h1 class="' . $titleClass . '" rel="' . $cufonRel . '">';
            if ($instance['themeton_widget_showhide'] == '1')
                $isShow = !$isShow;
            if (!$isShow)
