...
class TTSlideMetaBox {
	...
	if (isset($_POST['post_type']) && $this->metaConfiguration['type'] == $_POST['post_type']) {
		...
		for ($i = 0; $i < $counter; $i++) {
			...
			$customSliderMeta[] = Array('image' => stripslashes($_POST[$this->metaConfiguration['id']][$i]['image']), 'image_thumb' => $_POST[$this->metaConfiguration['id']][$i]['image_cropped'], 'media' => stripslashes($_POST[$this->metaConfiguration['id']][$i]['media']));
		    }
		}
		if($this->metaConfiguration['use_embed_video']&&isset($_REQUEST[$this->metaConfiguration['id'].'_embed_video'])){
		    if(get_post_meta($post_id, 'tt_slide_embed') == ''){
			add_post_meta($post_id, 'tt_slide_embed', $_REQUEST[$this->metaConfiguration['id'].'_embed_video'], true);
		    }else{
			update_post_meta($post_id, 'tt_slide_embed', $_REQUEST[$this->metaConfiguration['id'].'_embed_video']);
		    }
		}
...
?>
		...
		?>
    		<option ratio="<?php print $tval['ratio']; ?>" value="<?php print $tval['value']; ?>" <?php if ($tval['value'] == $size_meta)
			print 'selected'; ?> ><?php print $tkey; ?></option>
			<?php } ?>
            </select>
        </td>
    </tr>
...
<?php
	    if ($this->metaConfiguration['use_embed_video']) {
?>
		<div class="tt_embed">
		    <table class="form-table">
			<tr>
			    <th><label>If you want use video here embed code</label></th>
			    <td>
				<textarea name="<?php echo $this->metaConfiguration['id'] . '_embed_video'; ?>" rows="10" cols="50"><?php print get_post_meta($post->ID, 'tt_slide_embed', true); ?></textarea>
...
?>
        ...
        <tr class="featured_image_link rowitem">
            ...
            <td>
                <input type="text" name="<?php echo $this->metaConfiguration['id'] ?>[#index#][media]" id="<?php echo $this->metaConfiguration['id'] ?>_#index#_media" value="<?php print isset($option['media']) ? $option['media'] : ''; ?>" size="30" tabindex="30">
                <input class="button" rel="<?php echo $this->metaConfiguration['id'] ?>_#index#_media" type="button" value="Upload" >
                <small>It is provides your media (image or video) when click on your featured image.</small>
            </td>
        </tr>
    </table>
</div>
