...
class TTHeadSlideMetaBox {
    ...
    function construct($config) {
        ...
        add_action('wp_ajax_' . $this->metaConfiguration['id'] . '_crop', array(&$this, 'slide_image_crop'));
    }
            ...
            if (isset($_POST[$this->metaConfiguration['id'] . '_image_count_value']) && $_POST[$this->metaConfiguration['id'] . '_image_count_value'] != '0' || isset($_POST[$this->metaConfiguration['id'] . '_count_value']) && $_POST[$this->metaConfiguration['id'] . '_count_value'] != '') {
                ...
                for ($i = 0; $i < $counter; $i++) {
                    ...
                    if ($_POST[$this->metaConfiguration['id']][$i]['image'] != '') {
                        $customSliderMeta[] = Array('image' => stripslashes($_POST[$this->metaConfiguration['id']][$i]['image']), 'text' => $_POST[$this->metaConfiguration['id']][$i]['text'], 'link_text' => stripslashes($_POST[$this->metaConfiguration['id']][$i]['link_text']), 'link' => stripslashes($_POST[$this->metaConfiguration['id']][$i]['link']));
                    }
                }
                if (get_post_meta($post_id, 'tt_head_slide_images') == '') {
                    add_post_meta($post_id, 'tt_head_slide_images', $customSliderMeta, true);
                } else {
                    update_post_meta($post_id, 'tt_head_slide_images', $customSliderMeta);
                }
            } else {
                delete_post_meta($post_id, 'tt_head_slide_images');
            }
        }
		if(isset($_REQUEST['s_height'])){
			update_post_meta($post_id, 's_height', $_REQUEST['s_height']);
		}
    }
...
<input type="text" name="s_height" value="<?php print get_post_meta($post->ID, 's_height', true) ? get_post_meta($post->ID, 's_height', true) : '600'; ?>" size="6"/> px
<?php
...
?>
                ...
                //print_r($this->metaConfiguration['size']);
                foreach ($this->metaConfiguration['size']['values'] as $tkey => $tval) {
                
 ?>
                    <option ratio="<?php print $tval['ratio']; ?>" value="<?php print $tval['value']; ?>" <?php if($tval['value']==$size_meta) print 'selected'; ?> ><?php print $tkey; ?></option>
<?php } ?>
            </select>
        </td>
    </tr>
...
?>
    ...
    <?php
        ...
        $imgs = get_post_meta($post->ID, 'tt_head_slide_images', true);
        // print_r($imgs);
        if ($imgs != '')
            foreach ($imgs as $img) {
...
<?php if ($this->metaConfiguration['crop']) { ?>
            <div id="<?php echo $this->metaConfiguration['id']; ?>_cropper_content">
...
?>
                <input type="button" id="<?php echo $this->metaConfiguration['id']; ?>_add_button" class="add:the-list:newmeta button" tabindex="9" value="Add another"/>
                <script>
...
<?php
            ...
            function singleHtml($option) {
                $cropParams = '';
                if (isset($this->metaConfiguration['crop'])&&$this->metaConfiguration['crop']) {
                    if ($this->metaConfiguration['size']['ratio']) {
...
?>
                <div class="post-slider-image">
                    ...
                    <table class="form-table" width="100%">
                        <tr class="featured_image rowitem">
                            <th style="width:10%;">
                                <label for="featured_image">Slide image</label>
                            </th>
                            <td>
                                ...
                                <input type="hidden" name="<?php echo $this->metaConfiguration['id'] ?>[#index#][image_cropped]" id="<?php echo $this->metaConfiguration['id'] ?>_#index#_image_cropped" value="<?php print $option['image_thumb']; ?>" size="30" tabindex="30">
                                <input class="button" rel="<?php echo $this->metaConfiguration['id'] ?>_#index#_image" type="button" value="Upload" >
                <?php if ($this->metaConfiguration['crop']) {
                ?>
                ...
                <?php } ?>
                <small>Please select slider image</small>
            </td>
        </tr>
        <tr class="featured_image_link rowitem">
            <th style="width:10%;">
                <label for="featured_image_link">Slider text</label>
            </th>
            <td>
                <input type="text" name="<?php echo $this->metaConfiguration['id'] ?>[#index#][text]" id="<?php echo $this->metaConfiguration['id'] ?>_#index#_text" value="<?php print isset($option['text'])?$option['text']:''; ?>" size="30" tabindex="30">                
                <small>Add your slider text.</small>
            </td>
        </tr>
		<tr class="featured_image_link rowitem">
            <th style="width:10%;">
                <label for="featured_image_link">Slider link text</label>
            </th>
            <td>
                <input type="text" name="<?php echo $this->metaConfiguration['id'] ?>[#index#][link_text]" id="<?php echo $this->metaConfiguration['id'] ?>_#index#_link_text" value="<?php print isset($option['link_text'])?$option['link_text']:''; ?>" size="30" tabindex="30"/>
                <small>Link text.</small>
            </td>
        </tr>
		<tr class="featured_image_link rowitem">
            <th style="width:10%;">
                <label for="featured_image_link">Slider link</label>
            </th>
            <td>
                <input type="text" name="<?php echo $this->metaConfiguration['id'] ?>[#index#][link]" id="<?php echo $this->metaConfiguration['id'] ?>_#index#_link" value="<?php print isset($option['link'])?$option['link']:''; ?>" size="30" tabindex="30">                
