...
<?php
	...
	if (is_array($subscriptions) && !empty($subscriptions)){
		...
		foreach($subscriptions as $i => $a_subscription) {
			echo "<tr>";
			echo "<td><input type='checkbox' name='email_list[]' value='".urlencode($a_subscription->email)."' id='e_$i'/></td>
				<td><label for='e_$i'><span class='subscribe-column-1'>" . date('d.m.Y H:i' ,strtotime($a_subscription->dt)) . "</span></td>
					<td><span class='subscribe-column-2'>$a_subscription->status</span></td>
					<td><span class='subscribe-column-3'>$a_subscription->email</span></label></td>\n";
			echo "</tr>";
		}
		echo '</table>';
		...
		echo '<p id="subscribe-reloaded-update-p"><input type="submit" class="submit" value="'.__('Update subscriptions','subscribe-reloaded').'" /><input type="hidden" name="srp" value="'.intval($post_ID).'"/></p>';
