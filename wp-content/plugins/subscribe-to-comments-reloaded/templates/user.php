...
<?php
	...
	if (is_array($subscriptions) && !empty($subscriptions)){
		...
		foreach($subscriptions as $a_subscription){
			...
			$title = get_the_title($a_subscription->post_id);
			echo "<tr>";
			echo "<td><input type='checkbox' name='post_list[]' value='{$a_subscription->post_id}' id='post_{$a_subscription->post_id}'/></td>
			<td><label for='post_{$a_subscription->post_id}'><span class='subscribe-column-1'>" . date('d.m.Y H:i' ,strtotime($a_subscription->dt)) . "</span></label></td>
			<td><span class='subscribe-column-2'>{$a_subscription->status}</span>
			<td><a class='subscribe-column-3' href='$permalink'>$title</a></td>\n";
			echo "</tr>";
		}
		echo '</table>';
		...
		echo '<a class="subscribe-reloaded-small-button" href="#" onclick="t=document.forms[\'post_list_form\'].elements[\'post_list[]\'];c=t.length;if(!c){t.checked=!t.checked}else{for(var i=0;i<c;i++){t[i].checked=!t[i].checked}};return false;">'.__('Invert selection','subscribe-reloaded').'</a></p>';
		...
		echo '<p id="subscribe-reloaded-update-p"><input type="submit" class="subscribe-form-button submit" value="'.__('Update subscriptions','subscribe-reloaded').'" /><input type="hidden" name="sre" value="'.urlencode($email).'"/></p>';
